    ...
    public PrivateAccountsAssociationForCompanyGet fetchAssociationDetails(String companyNumber) {
        try {
            LOG.debug(String.format(
                    "Sending request to account-association-api: GET /associations/companies/{company_number}. Attempting to fetch Association details for company %s ",
                    companyNumber), null);

            return accountsAssociationEndpoint.buildGetAssociationsForCompanyRequest(companyNumber, false, 0, 1);
        } catch (ApiErrorResponseException exception) {
            if (exception.getStatusCode() == 404) {
                LOG.error(new Exception(String.format("Association not found for Company Number: %s ", companyNumber), exception),
                        null);
                throw new NotFoundRuntimeException(String.format("Association not found for Company Number: %s ", companyNumber),
                        exception);
            } else {
                LOG.error(new Exception(String.format("Failed to fetch for Company Number: %s ", companyNumber), exception),
                        null);
                throw new InternalServerErrorRuntimeException(
                        String.format("Failed to fetch for Company Number: %s", companyNumber), exception);
            }
        } catch (Exception exception) {
            LOG.error(new Exception(String.format("Failed to fetch for Company Number: %s", companyNumber), exception), null);
            throw new InternalServerErrorRuntimeException(
                    String.format("Unexpected error while fetching for Company Number: %s", companyNumber), exception);
        }
    }
    ...
    public void createUpdateStatusRequest(String associationId, StatusEnum statusEnum) {
        try {
            ...
            if (exception.getStatusCode() == 404) {
                ...
                LOG.error(
                        new Exception(String.format("Failed to update status for association ID: %s ", associationId), exception),
                        null);
                throw new InternalServerErrorRuntimeException(
                        String.format("Failed to update status for association ID: %s", associationId), exception);
            }
        } catch (Exception exception) {
            LOG.error(new Exception(String.format("Failed to update status for association ID: %s", associationId), exception),
                    null);
            throw new InternalServerErrorRuntimeException(
                    String.format("Unexpected error while updating status for association ID: %s", associationId), exception);
        }
    }
